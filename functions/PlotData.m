...
for i = 1:length(impacts)
    ...
    if confirmedImpactsExtracted == 1
        ...
        title({mpID;['Impact Date: ' date];['Impact Time: ' time];['Impact Index: ' num2str(index)];['Confirmed Impact: ' type];'';'Y Rot Vel'})
    else
        title({mpID;['Impact Date: ' date];['Impact Time: ' time];['Impact Index: ' num2str(index)];'';'Y Rot Vel'})
    end
